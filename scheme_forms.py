...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        evalu = scheme_eval(expressions.rest.first, env)
        env.define(signature, evalu)
        return signature
...
def do_and_form(expressions, env):
    ...
    """
    val = True
    scheme_list = expressions
    while scheme_list is not nil:
        val = scheme_eval(scheme_list.first, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        exp = bindings.first
        exprest = exp.rest
        names = Pair(exp.first, names)
        vals = Pair(scheme_eval(exprest.first, env), vals)
